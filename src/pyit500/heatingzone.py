...
from pyit500.zone import Zone, ZoneAttribute, ScheduleType, ActiveStatus, OnOffStatus
from pyit500.enumfriendly import EnumFriendly, IntEnumFriendly
...
class HeatingZone(Zone):
    ...
    async def async_set_current_setpoint(self, value: float) -> int:
        ...
        return await self.async_set_attr(ZoneAttribute.CH_CURRENT_SETPOINT, value * 100)
    ...
    async def async_set_manual_mode_setpoint(self, value: float) -> int:
        ...
        return await self.async_set_attr(
            ZoneAttribute.CH_MANUAL_MODE_SETPOINT, value * 100
        )
    ...
    async def async_set_auto_mode_setpoint(self, value: float) -> int:
        ...
        return await self.async_set_attr(
            ZoneAttribute.CH_AUTO_MODE_SETPOINT, value * 100
        )
    ...
    async def async_set_consolidated_mode(self, value: ConsolidatedMode):
        ...
        if off_mode != self.off_mode:
            print(f"Off should be {off_mode} but is {self.off_mode}")
            attr_to_set += (ZoneAttribute.CH_OFF_MODE, off_mode)
        if manual_mode != self.manual_mode:
            print(f"Manual should be {manual_mode} but is {self.manual_mode}")
            attr_to_set += (ZoneAttribute.CH_MANUAL_MODE, manual_mode)

        if auto_temp_hold_mode != self.auto_temp_hold_mode:
            print(
                f"Temp hold should be {auto_temp_hold_mode} but is {self.auto_temp_hold_mode}"
            )
            attr_to_set += (ZoneAttribute.CH_AUTO_TEMP_HOLD_MODE, auto_temp_hold_mode)

        return await self.async_set_attr(*attr_to_set)
